...
export async function POST(req: Request) {
    try {
        ...
        // Validação mínima (campos NOT NULL no schema)
        const missing: string[] = [];
        if (!body.nome) missing.push('nome');
        if (!body.preco) missing.push('preco');
        ...
        // id é NOT NULL (text) no schema. Gera se não vier no body.
        const id = typeof body.id === 'string' && body.id.trim().length > 0
            ? body.id.trim()
            : globalThis.crypto?.randomUUID?.() ?? `${Date.now()}`;

        // Monta o registro a inserir
        const novoPrato: Record<string, unknown> = {
            ...
            // imagens é ARRAY; envia somente se vier array
            ...(Array.isArray(body.imagens) ? { imagens: body.imagens } : {}),
            // isActive default true no DB; permite override se vier boolean
        ...
        if (error) {
            console.error('POST /pratos error:', error);
            return NextResponse.json(
                { error: 'DB error', details: error.message },
                { status: 500 }
            );
